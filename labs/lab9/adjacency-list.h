...
void AdjList::problem2(std::string fname){
    ...
    for(unsigned int i = 0; i < this->adjList.size(); i++){
		for(std::list<std::pair<unsigned int, unsigned int>>::iterator it = this->adjList[i].begin(); it != this->adjList[i].end(); ++it){
			std::string color = (*it).second % 2 == 0 ? "blue" : "red";
            of << "\t" << i << " -> " << (*it).first << " [label=\"" << (*it).second << "\"][color=\"" << color << "\"]" << ";" << std::endl;
		}
	}
...
void AdjList::problem3(std::string fname){
    ...
    for(unsigned int i = 0; i < this->adjList.size(); i++){
		for(std::list<std::pair<unsigned int, unsigned int>>::iterator it = this->adjList[i].begin(); it != this->adjList[i].end(); ++it){
			// Get index of an iterator: https://stackoverflow.com/a/2153006
            std::string color = colors[std::distance(this->adjList[i].begin(), it)];
            of << "\t" << i << " -> " << (*it).first << " [label=\"" << (*it).second << "\"][color=\"" << color << "\"]" << ";" << std::endl;
		}
	}
...
void AdjList::DFS(std::ofstream& of, unsigned int& vertex, bool visited[], int depth){
    ...
    // DFS skeleton code: https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/
    visited[vertex] = true;

    for (auto &adj_vertex : adjList[vertex]) {
        of << "\t" << vertex << " -> " << adj_vertex.first << " [label=\"" << adj_vertex.second << "\"][color=\"" << colors[depth] << "\"]" << ";" << std::endl;
        if (!visited[adj_vertex.first]) {
            DFS(of, adj_vertex.first, visited, depth + 1);
        }
    }
}
